  ...
  return (
    <Link href={`/books/${book._id}`}>
      <article className="group overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm hover:shadow-xl transition-all duration-300">
        ...
        {/* Content */}
        <div className="p-5">
          <h3 className="text-xl font-bold text-zinc-900 line-clamp-1">{book.title}</h3>
          <p className="text-zinc-500 mt-1">by {book.authorName}</p>
          ...
          <div className="mt-5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Bookmark className="size-4 text-zinc-500" />
              <button className="flex items-center gap-1.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium px-3 py-1.5 rounded-xl transition-colors">
                <ShoppingCart className="size-4" />
                Buy
              </button>
            </div>
